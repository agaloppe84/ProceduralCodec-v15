...
class SpiralSquare(Generator):
    @property
    def info(self) -> GeneratorInfo:
        return GeneratorInfo(
            name="SPIRAL_SQUARE",
            param_specs=(
                ParamSpec("turns", "float", (0.5, 32.0), None, 0.5),
                ParamSpec("tight", "float", (0.1, 5.0), None, 0.1),
            ),
    ...
    def render(self, tiles_hw, params, seeds, *, device, dtype):
        ...
        turns=params[:,0].view(B,1,1).to(dtype)
        tight=params[:,1].view(B,1,1).to(dtype)
        r_inf = torch.maximum(torch.abs(xx), torch.abs(yy))
        theta = torch.atan2(yy, xx) + torch.pi
        s = torch.sin(tight * r_inf + turns * theta)
