...
class ValueNoise(Generator):
    ...
    def info(self)->GeneratorInfo:
        return GeneratorInfo(name="VALUE_NOISE",
    ...
    def render(self,tiles_hw,params,seeds,*,device,dtype):
        ...
        for k in range(6):
            use=(k<octv).to(dtype); v=value_noise2d(xx,yy,freq,seed64+k)
            val=val+v*amp*use; amp=amp*pers; freq=freq*2.0
        sum_amp=(1.0 - torch.pow(pers, octv.to(dtype)))/(1.0 - pers + 1e-6)
        out=(val/(sum_amp+1e-6))*2.0-1.0; return out.clamp(-1,1).unsqueeze(1)
GEN=ValueNoise()
